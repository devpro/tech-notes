...
- Control nodes
  ...
  > Ansible commands and playbooks can be run by invoking the ansible or ansible-playbook command from any control node (any computer that has a Python installation).
  > Having multiple control nodes is possible.

- Managed nodes

  > The network devices (and/or servers) managed with Ansible.
...
- [Inventory](./ansible-inventory.md)
  ...
  > The inventory can specify information like IP address for each managed node.
...
- [Modules](./ansible-modules.md)
  ...
  > A single module with a task can be invoked, or several different modules in a playbook.
  > Starting in Ansible 2.10, modules are grouped in collections.

- Tasks

  > The units of action in Ansible.
  > A single task can be executed once with an ad hoc command.

- [Playbooks](./ansible-playbooks.md)

  > Ordered lists of tasks, saved so that those tasks in that order repeatedly.
